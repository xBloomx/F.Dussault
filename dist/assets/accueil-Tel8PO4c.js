import{t as e}from"./supabase-BHP_DPH_.js";import{c as t,i as n,n as r,r as i,t as a}from"./auth-wlrJYOzK.js";import{t as o}from"./sanitize-CY3yUbkZ.js";var s={factures:`facture`,temps:`feuilleTemps`,feuilles:`feuilleTemps`,soumissions:`soumissions`,messagerie:`messagerie`,courriel:`courriel`,calendrier:`calendrier`,clients:`clients`,po:`po`,outils:`outils`,admin:`admin`,profil:`profil`,accueil:`accueil`};function c(e){let t=s[e]||e,n=document.querySelector(`.nav-btn[data-view="${t}"]`);n&&!n.classList.contains(`hidden`)&&n.click()}var l=[],u=null;async function d(e){e.innerHTML=`
    <style>
        .main-content {
    ...
    `,await f()}async function f(){let e=new Date().toLocaleDateString(`fr-CA`,{weekday:`long`,year:`numeric`,month:`long`,day:`numeric`});document.getElementById(`todayDate`).textContent=e.charAt(0).toUpperCase()+e.slice(1);let t=a?.prenom_nom?.split(` `)[0]||`vous`;document.getElementById(`greeting`).textContent=`Bonjour, ${t} !`;let i={A0:`<svg width="16" height="16" style="stroke:currentColor;fill:none;stroke-width:2"><use href="#ic-code"/></svg> DÉVELOPPEUR SYSTÈME`,A1:`<svg width="16" height="16" style="stroke:currentColor;fill:none;stroke-width:2"><use href="#ic-crown"/></svg> Administrateur`,A2:`<svg width="16" height="16" style="stroke:currentColor;fill:none;stroke-width:2"><use href="#ic-building"/></svg> Gestion &amp; Bureau`,A3:`<svg width="16" height="16" style="stroke:currentColor;fill:none;stroke-width:2"><use href="#ic-briefcase"/></svg> Employé sur le terrain`};document.getElementById(`roleText`).innerHTML=i[r]||i.A3,n(`manage_news`)?document.getElementById(`btn-add-news`).style.display=`flex`:document.getElementById(`btn-add-news`).style.display=`none`,document.querySelectorAll(`.quick-action-card[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>c(e.dataset.nav))}),document.querySelectorAll(`.stat-item[data-nav]`).forEach(e=>{e.addEventListener(`click`,()=>c(e.dataset.nav))}),document.getElementById(`btn-add-news`).addEventListener(`click`,g),document.getElementById(`btnCancelNews`).addEventListener(`click`,_),document.getElementById(`btnSaveNews`).addEventListener(`click`,v),document.getElementById(`btnCancelConfirm`).addEventListener(`click`,C),document.getElementById(`btnYesConfirm`).addEventListener(`click`,()=>{u&&u(),C()}),document.getElementById(`pinnedGroup`).addEventListener(`click`,e=>{e.target.id!==`newsPinned`&&document.getElementById(`newsPinned`).click()}),await Promise.all([p(),x(),b()]);let o=()=>x();window.addEventListener(`formations_updated`,o);let s=()=>b();return window.addEventListener(`focus`,s),function(){window.removeEventListener(`formations_updated`,o),window.removeEventListener(`focus`,s)}}async function p(){try{let{data:t,error:n}=await e.from(`annonces`).select(`*`).order(`is_pinned`,{ascending:!1}).order(`created_at`,{ascending:!1});if(n)throw n;l=(t||[]).map(e=>({id:e.id,type:e.type,title:e.titre,body:e.contenu,author:e.auteur,date:e.created_at,isPinned:e.is_pinned}))}catch{l=[]}m()}function m(){let e=document.getElementById(`newsList`);if(e){if(e.innerHTML=``,l.length===0){e.innerHTML=`<div style="background:var(--bg-panel);border-radius:12px;border:1px dashed #444;color:#888;text-align:center;padding:40px;font-style:italic">Aucune annonce pour le moment.</div>`;return}[...l].sort((e,t)=>e.isPinned&&!t.isPinned?-1:!e.isPinned&&t.isPinned?1:new Date(t.date)-new Date(e.date)).forEach(t=>{let r=new Date(t.date),i=r.toLocaleDateString(`fr-CA`)+` à `+r.toLocaleTimeString(`fr-CA`,{hour:`2-digit`,minute:`2-digit`}),a=``;if(n(`manage_news`)){let e=t.isPinned?`pin-active`:``,n=t.isPinned?`Désépingler`:`Épingler en haut`;a=`
                ...
                </div>`}let s=`ic-info`;t.type===`important`&&(s=`ic-alert-circle`),t.type===`normal`&&(s=`ic-edit`),t.isPinned&&(s=`ic-pin`);let c=document.createElement(`div`);c.className=`news-card ${t.type} ${t.isPinned?`pinned`:``}`,c.innerHTML=`
            <div class="news-header">
                <h3 class="news-title">
                    <svg width="20" height="20" style="stroke:currentColor;fill:none;stroke-width:2;margin-top:2px"><use href="#${o(s)}"/></svg>
                    ${o(t.title)}
                </h3>
                <div class="news-date">
            ...
            <div class="news-body">${o(t.body)}</div>
            <div class="news-author">Auteur : <strong>${o(t.author)}</strong></div>
            ${a}
        `,e.appendChild(c)}),e.querySelectorAll(`[data-pin]`).forEach(e=>{e.addEventListener(`click`,()=>h(e.dataset.pin))}),e.querySelectorAll(`[data-delete]`).forEach(e=>{e.addEventListener(`click`,()=>y(e.dataset.delete))})}}async function h(t){let n=l.findIndex(e=>e.id===t);n>-1&&(l[n].isPinned=!l[n].isPinned,await e.from(`annonces`).update({is_pinned:l[n].isPinned}).eq(`id`,t),m())}function g(){if(!n(`manage_news`)){t(`Vous n'avez pas la permission.`,`warning`);return}document.getElementById(`newsTitle`).value=``,document.getElementById(`newsBody`).value=``,document.getElementById(`newsType`).value=`info`,document.getElementById(`newsPinned`).checked=!1,document.getElementById(`newsModal`).classList.add(`open`)}function _(){document.getElementById(`newsModal`).classList.remove(`open`)}async function v(){if(!n(`manage_news`))return;let i=document.getElementById(`newsTitle`).value.trim(),o=document.getElementById(`newsBody`).value.trim(),s=document.getElementById(`newsType`).value,c=document.getElementById(`newsPinned`).checked;if(!i||!o){t(`Veuillez remplir le titre et le message.`,`warning`);return}let l=document.getElementById(`btnSaveNews`);if(l?.disabled)return;l&&(l.disabled=!0,l.style.opacity=`0.6`);let u=a?.prenom_nom||`Utilisateur`;if(r===`A0`)try{localStorage.getItem(`dussault_a0_alias`)===`systeme`&&(u=`Système`)}catch{}try{let{error:n}=await e.from(`annonces`).insert([{type:s,titre:i,contenu:o,auteur:u,is_pinned:c}]);if(n){t(`Erreur de publication : `+n.message,`error`);return}await p(),_()}finally{l&&(l.disabled=!1,l.style.opacity=``)}}function y(n){S(`Voulez-vous vraiment supprimer cette annonce pour tout le monde ?`,async()=>{let{error:r}=await e.from(`annonces`).delete().eq(`id`,n);if(r){t(`Erreur suppression : `+r.message,`error`);return}l=l.filter(e=>e.id!==n),m()})}async function b(){if(i)try{let[t,n,r]=await Promise.all([e.from(`factures`).select(`*`,{count:`exact`,head:!0}).eq(`author_id`,i.id).in(`status`,[`brouillon`,`renvoye`]),e.from(`feuilles_de_temps`).select(`*`,{count:`exact`,head:!0}).eq(`author_id`,i.id).eq(`status`,`brouillon`),e.from(`outils`).select(`*`,{count:`exact`,head:!0}).eq(`assignee_nom`,a?.prenom_nom).eq(`status`,`active`)]),o=t.count??0,s=n.count??0,c=r.count??0,l=document.getElementById(`item-inv`),u=document.getElementById(`item-ts`),d=document.getElementById(`item-tools`),f=document.getElementById(`bilan-empty-msg`);l&&(document.getElementById(`stat-inv`).textContent=o,l.style.display=o>0?`flex`:`none`),u&&(document.getElementById(`stat-ts`).textContent=s,u.style.display=s>0?`flex`:`none`),d&&(document.getElementById(`stat-tools`).textContent=c,d.style.display=c>0?`flex`:`none`),f&&(f.style.display=o===0&&s===0&&c===0?`block`:`none`)}catch(e){console.error(`Erreur stats:`,e)}}async function x(){let t=document.getElementById(`alertsList`);if(!t||!i)return;t.innerHTML=``;let n=!1,{data:r}=await e.from(`formations`).select(`*`).eq(`user_id`,i.id),a=r||[],s=new Date;a.forEach(e=>{let r=new Date(e.date_expiration),i=Math.ceil((r-s)/(1e3*60*60*24)),a=o(e.nom||e.name||`Formation`);i<0?(n=!0,t.insertAdjacentHTML(`beforeend`,`
                ...
                </div>`)):i<=30&&(n=!0,t.insertAdjacentHTML(`beforeend`,`
