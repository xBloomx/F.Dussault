import{t as e}from"./supabase-BHP_DPH_.js";import{i as t,n,r,t as i}from"./auth-wlrJYOzK.js";import{t as a}from"./offlineQueue-DvhPUZVs.js";import{t as o}from"./withRetry-DqM7w1cb.js";import{a as s,c,i as l,n as u,o as d,r as f,s as p,t as m}from"./zoom-D_OkEY6M.js";var h=`Employé`,g=`mine`,_=[],v=null,y=null,b=0,x=25,S=!1,C=null,w=null,T=null;function E(){return t(`view_all_timesheets`)||t(`approve_timesheets`)}async function D(e){return h=i?.prenom_nom||`Employé`,e.innerHTML=`
    <style>
    ...
    `,await O(e),k}async function O(e){let t=e.querySelector(`#view-dashboard`),n=e.querySelector(`#view-editor`),r=e.querySelector(`#zoom-wrapper`),i=e.querySelector(`#zoom-level`);E()?e.querySelector(`#timesheet-tabs`).style.display=`flex`:(e.querySelector(`#timesheet-tabs`).style.display=`flex`,e.querySelector(`#tab-all`).style.display=`none`),e.querySelector(`#tab-mine`).addEventListener(`click`,()=>j(`mine`,e)),e.querySelector(`#tab-all`).addEventListener(`click`,()=>j(`all`,e)),e.querySelector(`#tab-archives`).addEventListener(`click`,()=>j(`archives`,e)),e.querySelector(`#searchInput`).addEventListener(`keyup`,()=>N(e)),e.querySelector(`#btnNewSheet`).addEventListener(`click`,()=>P(t,n,r,i,e)),e.querySelector(`#btnBack`).addEventListener(`click`,()=>L(r,t,n,e)),e.querySelector(`#btnSave`).addEventListener(`click`,()=>B(!1,r,t,n,e)),e.querySelector(`#btnSend`).addEventListener(`click`,()=>{Z(`Une fois envoyée, vous ne pourrez plus la modifier. L'envoyer pour la paie ?`,()=>B(!0,r,t,n,e),e,`Envoyer au bureau`)}),e.querySelector(`#btnApprove`).addEventListener(`click`,()=>V(t,n,e)),e.querySelector(`#btnReturn`).addEventListener(`click`,()=>{e.querySelector(`#refusNote`).value=``,e.querySelector(`#refusModal`).classList.add(`open`)}),e.querySelector(`#btnUnlock`).addEventListener(`click`,()=>z(r,e)),e.querySelector(`#btnPdf`).addEventListener(`click`,()=>U(r)),e.querySelector(`#btnAddPage`).addEventListener(`click`,()=>{let e=W();r.appendChild(e),G(e),w?.applyZoom(w?.current??1)}),e.querySelector(`#btnDupPage`).addEventListener(`click`,()=>K(r,i)),e.querySelector(`#btnDelPage`).addEventListener(`click`,()=>q(r,e)),e.querySelector(`#btnClear`).addEventListener(`click`,()=>{Z(`Effacer tout le contenu ?`,()=>{r.querySelectorAll(`input`).forEach(e=>e.value=``)},e)}),w=m({container:r,scrollArea:e.querySelector(`#scrollArea`),zoomDisplay:i,docWidthPx:816}),w.attach(),e.querySelector(`#btnZoomOut`).addEventListener(`click`,()=>w.zoomOut()),e.querySelector(`#btnZoomIn`).addEventListener(`click`,()=>w.zoomIn()),e.querySelector(`#btnZoomReset`).addEventListener(`click`,()=>w.zoomReset()),T=()=>{n.style.display===`flex`&&w?.fitToScreen()},window.addEventListener(`resize`,T),e.querySelector(`#btnConfirmNo`).addEventListener(`click`,()=>Q(e)),e.querySelector(`#btnConfirmYes`).addEventListener(`click`,()=>{C&&C(),Q(e)}),e.querySelector(`#btnAlertOk`).addEventListener(`click`,()=>e.querySelector(`#alertModal`).classList.remove(`open`)),e.querySelector(`#btnCloseRefus`).addEventListener(`click`,()=>e.querySelector(`#refusModal`).classList.remove(`open`)),e.querySelector(`#btnConfirmRefus`).addEventListener(`click`,()=>H(t,n,e)),await A(!0,e)}function k(){Y(),w&&=(w.destroy(),null),T&&=(window.removeEventListener(`resize`,T),null)}async function A(t=!0,i){t&&(b=0,_=[]);let a=b*x,o=a+x-1,c=e.from(`feuilles_de_temps`).select(`id, employe_nom, periode, status, total_heures, author_id, author_name, return_note, is_archived`);g===`archives`?(c=c.eq(`is_archived`,!0),s(n)||(c=c.eq(`author_id`,r.id))):(c=c.eq(`is_archived`,!1),c=g===`mine`?c.eq(`author_id`,r.id):c.neq(`status`,`brouillon`));let{data:l}=await c.order(`created_at`,{ascending:!1}).range(a,o+1);if(l){S=l.length>x;let e=l.slice(0,x).map(e=>({id:e.id,employe:e.employe_nom,periode:e.periode,status:e.status,pagesData:[],total_heures:e.total_heures||0,authorId:e.author_id,authorName:e.author_name,return_note:e.return_note,isArchived:e.is_archived===!0}));_=t?e:[..._,...e]}N(i)}function j(e,t){g=e,t.querySelectorAll(`.btn-tab`).forEach(e=>e.classList.remove(`active`)),t.querySelector(`#tab-${e}`)?.classList.add(`active`),A(!0,t)}function M(e,t){let i=t.querySelector(`#timesheetListContainer`);i.innerHTML=``;let a=E(),o=t.querySelector(`#ts-compteur`);if(o&&(o.textContent=`${_.length} feuille(s) chargée(s)${S?` — il y en a plus`:``}`),e.length===0){i.innerHTML=`<div style="color:#888;text-align:center;padding:20px;font-style:italic">Aucune feuille trouvée.</div>`;return}if(e.forEach(e=>{let o=``;e.isArchived?o=`<span class="badge-status" style="background:#555">Archivée</span>`:e.status===`brouillon`?o=`<span class="badge-status b-brouillon">Brouillon</span>`:e.status===`envoye`?o=`<span class="badge-status b-envoye">${a?`En attente`:`Envoyée`}</span>`:e.status===`approuve`?o=`<span class="badge-status b-paye">Approuvée</span>`:e.status===`renvoye`&&(o=`<span class="badge-status b-renvoye">Renvoyée</span>`);let s=`<div style="width:36px"></div>`;e.isArchived?l(n)&&(s=`<button class="btn-icon" style="background:rgba(40,167,69,0.15);color:#28a745" data-restore="${e.id}">↺</button>`):f(e,n,r.id).allowed&&(s=`<button class="btn-icon btn-delete" data-delete="${e.id}">
            ...
            </button>`);let c=document.createElement(`div`);c.className=`invoice-item`,c.innerHTML=`
        ...
        `,c.addEventListener(`click`,n=>{n.target.closest(`[data-delete],[data-restore]`)||F(e.id,t)}),c.querySelector(`[data-delete]`)?.addEventListener(`click`,i=>{i.stopPropagation(),d({table:`feuilles_de_temps`,id:e.id,item:e,role:n,userId:r.id,userName:h,onSuccess:()=>A(!0,t),showConfirm:(e,n)=>Z(e,n,t),showAlert:e=>$(e,t)})}),c.querySelector(`[data-restore]`)?.addEventListener(`click`,r=>{r.stopPropagation(),p({table:`feuilles_de_temps`,id:e.id,role:n,onSuccess:()=>A(!0,t),showConfirm:(e,n)=>Z(e,n,t),showAlert:e=>$(e,t)})}),i.appendChild(c)}),S){let e=document.createElement(`button`);e.textContent=`Charger ${x} feuilles de plus...`,e.style.cssText=`width:100%;padding:14px;margin-top:10px;background:var(--bg-panel);color:var(--text-muted);border:1px dashed var(--border);border-radius:10px;cursor:pointer;font-size:14px;font-weight:bold`,e.addEventListener(`click`,async()=>{if(!e.disabled){e.disabled=!0,e.textContent=`Chargement...`,b++;try{await A(!1,t)}catch(t){console.error(`[feuilleTemps] Erreur pagination:`,t?.message),b--,e.disabled=!1,e.textContent=`Charger ${x} feuilles de plus...`}}}),i.appendChild(e)}}function N(e){let t=e.querySelector(`#searchInput`)?.value.toLowerCase()||``,n=E();M((g===`archives`?_:!n||g===`mine`?_.filter(e=>e.authorId===r.id||!e.authorId):_.filter(e=>e.status!==`brouillon`)).filter(e=>(e.employe||e.authorName||``).toLowerCase().includes(t)||(e.periode||``).toLowerCase().includes(t)),e)}function P(e,t,n,r,i){try{localStorage.removeItem(`fdussault_draft_feuille_de_temps_new`)}catch{}v=null,e.style.display=`none`,t.style.display=`flex`,n.innerHTML=``;let a=W();n.appendChild(a),G(a),R(null,n,i),w?.fitToScreen(),J(n)}async function F(t,n){let r=_.find(e=>e.id===t);if(!r)return;v=t;let i=n.querySelector(`#view-dashboard`),a=n.querySelector(`#view-editor`),o=n.querySelector(`#zoom-wrapper`);n.querySelector(`#zoom-level`),i.style.display=`none`,a.style.display=`flex`,o.innerHTML=``;let{data:s}=await e.from(`feuilles_de_temps`).select(`pages_data`).eq(`id`,t).single(),c=s?.pages_data||[];if(c.length)c.forEach(e=>{let t=W();o.appendChild(t),G(t);let n=t.querySelectorAll(`input`);e.forEach((e,t)=>{n[t]&&(n[t].value=e)})});else{let e=W();o.appendChild(e),G(e)}R(r,o,n),w?.fitToScreen(),!r.isArchived&&(!r.status||r.status===`brouillon`)&&J(o)}function I(e,t,n){Y(),e.style.display=`flex`,t.style.display=`none`,A(!0,n)}function L(e,t,n,r){y?.hasDraft?.()?Z(`Des modifications non sauvegardées seront perdues. Quitter quand même ?`,()=>I(t,n,r),r,`Modifications non sauvegardées`):I(t,n,r)}function R(e,n,i){let a=t(`approve_timesheets`),o=e?.status||`brouillon`,s=e?e.authorId===r.id||!e.authorId:!0,c=e?.isArchived===!0,l=s&&(o===`brouillon`||o===`renvoye`||o===`attente`);c&&(l=!1);let u=(e,t)=>{let n=i.querySelector(e);n&&(n.style.display=t?`flex`:`none`)};u(`#btnSave`,l),u(`#btnSend`,l),u(`#btnAddPage`,l),u(`#btnDupPage`,l),u(`#btnDelPage`,l),u(`#btnClear`,l),u(`#btnUnlock`,a&&o!==`brouillon`&&o!==`renvoye`&&o!==`attente`&&!c),u(`#btnApprove`,!c&&a&&(o===`envoye`||o===`attente`)),u(`#btnReturn`,!c&&a&&(o===`envoye`||o===`attente`));let d=i.querySelector(`#note-refus-box`),f=i.querySelector(`#note-refus-text`);d&&e?.return_note?(d.style.display=`block`,f&&(f.textContent=e.return_note)):d&&(d.style.display=`none`),n.querySelectorAll(`input`).forEach(e=>{l?(e.removeAttribute(`readonly`),e.style.pointerEvents=`auto`):(e.setAttribute(`readonly`,!0),e.style.pointerEvents=`none`)})}function z(e,t){e.querySelectorAll(`input`).forEach(e=>{e.removeAttribute(`readonly`),e.style.pointerEvents=`auto`}),[`#btnSave`,`#btnAddPage`,`#btnDupPage`,`#btnDelPage`,`#btnClear`].forEach(e=>{let n=t.querySelector(e);n&&(n.style.display=`flex`)}),t.querySelector(`#btnUnlock`).style.display=`none`,$(`Feuille de temps débloquée. N'oubliez pas de sauvegarder.`,t)}async function B(t,n,i,s,c){let l=n.querySelector(`.page`);if(!l)return;let u=c.querySelector(`#btnSave`),d=u?.innerHTML;u&&(u.disabled=!0,u.textContent=`Sauvegarde...`),n.querySelectorAll(`.page`).forEach(e=>{e.querySelectorAll(`table.temps-table tbody`).forEach(e=>{let t=Array.from(e.querySelectorAll(`tr`)),n=t.map(e=>{let t=Array.from(e.querySelectorAll(`input`)).map(e=>e.value);return{values:t,isEmpty:t.every(e=>!e?.trim())}}),r=[...n.filter(e=>!e.isEmpty),...n.filter(e=>e.isEmpty)];t.forEach((e,t)=>{e.querySelectorAll(`input`).forEach((e,n)=>{e.value=r[t].values[n]||``})})})});let f=l.querySelector(`.input-nom`),p=l.querySelector(`.input-semaine-debut`),m=l.querySelector(`.input-semaine-fin`),g=f?.value.trim()||h;f&&!f.value.trim()&&(f.value=g);let y=p?.value&&m?.value?`Du ${p.value} au ${m.value}`:`Semaine en cours`,b=[];n.querySelectorAll(`.page`).forEach(e=>{b.push(Array.from(e.querySelectorAll(`input`)).map(e=>e.value))});let x=0;n.querySelectorAll(`.heure-input`).forEach(e=>{let t=parseFloat(e.value);isNaN(t)||(x+=t)});let S=v||`TS-`+Date.now(),C=_.find(e=>e.id===v||e.id===S),w=C?.status||`brouillon`;t&&(w=`envoye`);let T={id:S,employe_nom:g,periode:y,pages_data:b,total_heures:x,status:w,author_id:C?C.authorId:r.id,author_name:C?C.authorName:h},{error:E}=await o(()=>e.from(`feuilles_de_temps`).upsert(T));if(u&&(u.disabled=!1,u.innerHTML=d),E){if(E.offline){a(`feuilles_de_temps`,T),$(`Hors ligne — la feuille sera synchronisée automatiquement à la reconnexion.`,c);return}$((E.message||``).toLowerCase().includes(`lock broken`)?`❌ Réessayez dans 2 secondes.`:`❌ Erreur : `+E.message,c);return}v=S,X();try{localStorage.removeItem(`fdussault_draft_feuille_de_temps_new`)}catch{}await A(!0,c),t?($(`Feuille de temps envoyée au bureau avec succès !`,c),I(i,s,c)):($(`Feuille de temps sauvegardée !`,c),R(T,n,c))}async function V(t,n,r){v&&Z(`Approuver cette feuille de temps ?`,async()=>{let{error:i}=await e.from(`feuilles_de_temps`).update({status:`approuve`,return_note:null}).eq(`id`,v);if(i){$(`❌ Erreur : `+i.message,r);return}await A(!0,r),I(t,n,r),$(`Feuille de temps approuvée !`,r)},r)}async function H(t,n,r){let i=r.querySelector(`#refusNote`)?.value.trim();if(!i){r.querySelector(`#refusNote`)&&(r.querySelector(`#refusNote`).style.borderColor=`var(--btn-red)`);return}if(!v)return;let{error:a}=await e.from(`feuilles_de_temps`).update({status:`renvoye`,return_note:i}).eq(`id`,v);if(a){$(`❌ Erreur : `+a.message,r);return}r.querySelector(`#refusModal`).classList.remove(`open`),await A(!0,r),I(t,n,r),$(`↩️ Feuille renvoyée à l'employé pour correction.`,r)}function U(e){let t=e.querySelector(`.page`);if(!t)return;let n=t.querySelector(`.input-nom`),r=t.querySelector(`.input-semaine-debut`),i=n?.value.trim()||h,a=r?.value.trim()||``,o=a.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);o&&(a=`${o[3]}-${o[2].padStart(2,`0`)}-${o[1].padStart(2,`0`)}`),a||=new Date().toISOString().split(`T`)[0],u({container:e,docType:`feuille`,docNumber:v||null,clientName:i,date:a})}function W(){let e=document.createElement(`div`);e.className=`page`;let t=``;for(let e=0;e<15;e++)t+=`<tr>
        ...
        <div class="ts-emp-info">
            <div class="ts-row">
                ...
                <input type="text" class="input-nom ts-field" value="${h}">
            </div>
